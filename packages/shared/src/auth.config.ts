...
export const CMS_WRITE_ROLES: UserRole[] = [ROLES.SUPER_ADMIN, ROLES.ADMIN];
  ...
  callbacks: {
    ...
    async jwt({ token, user }) {
      if (user) {
        ...
        token.role = (user as { role?: string }).role ?? ROLES.USER;
      }
      return token;
    },
    /**
...
export async function buildFullAuthConfig(): Promise<NextAuthConfig> {
